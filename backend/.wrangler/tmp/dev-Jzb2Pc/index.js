...
// .wrangler/tmp/bundle-FaxwS1/checked-fetch.js
...
// .wrangler/tmp/bundle-FaxwS1/strip-cf-connecting-ip-header.js
...
var app = new Hono2();
app.use("*", cors());
async function fetchCached(url, init, timeoutMs = 8e3) {
...
app.get("/api/context/:iso2/:iso3", async (c) => {
  ...
  const extractWB = /* @__PURE__ */ __name((data) => data && data[1] && data[1].length > 0 ? data[1][0].value : null, "extractWB");
  const extractWHO = /* @__PURE__ */ __name((data) => data && data.value && data.value.length > 0 ? data.value[0].NumericValue : null, "extractWHO");
  let restCountryData = null;
  if (restCountriesResp && restCountriesResp.data && Array.isArray(restCountriesResp.data)) {
    const match2 = restCountriesResp.data.find((c2) => c2.codes?.alpha_3 === iso3) || restCountriesResp.data[0];
    if (match2) {
      restCountryData = {
        capital: match2.capitals?.[0]?.name ? [match2.capitals[0].name] : void 0,
        region: match2.region,
        population: match2.population
      };
    }
  }
  return c.json({
...
// .wrangler/tmp/bundle-FaxwS1/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-FaxwS1/middleware-loader.entry.ts
